...
class RCNNHead(nn.Module):
    ...
    def forward(self, features, bboxes, pro_features, pooler, time_emb):
        ...
        # attention_map = self.spatial_attention(roi_features)
        # roi_features = roi_features * attention_map.unsqueeze(1)  # Assuming attention_map is [N, 1, H, W]
        ...
        pro_features2 = self.self_attn1(pro_features, pro_features, value=pro_features)[0]
        pro_features = pro_features + self.dropout1(pro_features2)
        pro_features = self.norm1(pro_features)
        
        #self_att2.
        pro_features = self.self_attn2(pro_features, pro_features, value=pro_features)[0]
        pro_features = self.norm2(pro_features) + self.dropout2(pro_features)
...
class SpatialAttention(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(in_channels=256, out_channels=64, kernel_size=1)  # example dimensions
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = nn.Conv2d(64, 1, kernel_size=1)
        self.sigmoid = nn.Sigmoid()

    def forward(self, x):
        # x is the feature map with shape [batch_size, channels, height, width]
        x = self.conv1(x)
        x = self.relu(x)
        x = self.conv2(x)
        attention_map = self.sigmoid(x)  # This will have the same spatial dimensions as input
